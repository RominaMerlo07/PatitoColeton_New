...
{
    public partial class InformeProgreso : Form
    {

        logMaterias logMaterias = new logMaterias();

        public InformeProgreso(int idAlumno, int etapa)
        {
            InitializeComponent();
            DataSet informe = logMaterias.InformeProgreso(idAlumno, etapa);
            var reportPath = "GestionJardin.Reporte.Alumnos.InformeProgreso.rdlc";

            ReportDataSource sReportDataSource = new ReportDataSource();
            this.reportViewer1.LocalReport.ReportEmbeddedResource = reportPath;
            sReportDataSource.Name = "InformeProgresos";
