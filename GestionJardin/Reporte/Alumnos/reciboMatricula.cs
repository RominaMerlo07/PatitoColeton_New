...
{
    public partial class reciboMatricula : Form
    {
        logCuota logCuota = new logCuota();

        public reciboMatricula(int idAlumno)
        {
            InitializeComponent();
            DataSet informe = logCuota.reciboMatricula(idAlumno);
            var reportPath = "GestionJardin.Reporte.Alumnos.reciboMatricula.rdlc";

            ReportDataSource sReportDataSource = new ReportDataSource();
            this.reportViewer1.LocalReport.ReportEmbeddedResource = reportPath;
            sReportDataSource.Name = "ReciboMat";
